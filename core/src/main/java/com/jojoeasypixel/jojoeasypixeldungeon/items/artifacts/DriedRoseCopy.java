...
import com.jojoeasypixel.jojoeasypixeldungeon.effects.particles.ShaftParticle;
...
import com.jojoeasypixel.jojoeasypixeldungeon.messages.Languages;
...
import com.jojoeasypixel.jojoeasypixeldungeon.sprites.HorseSprite;
...
import com.jojoeasypixel.jojoeasypixeldungeon.windows.WndBlacksmith;
import com.jojoeasypixel.jojoeasypixeldungeon.windows.WndQuest;
...
public class DriedRoseCopy extends Artifact {

	{
		image = ItemSpriteSheet.ARTIFACT_SLOWDANCER;
	...
	public void charge(Hero target) {
		if (ghost == null && charge < chargeCap){
			...
			if (partialCharge >= 1){
				...
				if (charge == chargeCap){
					partialCharge = 0f;
					GLog.p( Messages.get(DriedRoseCopy.class, "charged") );
				}
			}
		}
	}
	...
	public static void holdGhostHero( Level level ){
		for (Mob mob : level.mobs.toArray( new Mob[0] )) {
			if (mob instanceof DriedRoseCopy.GhostHero) {
	...
	public static class Petal extends Item {
		...
		public boolean doPickUp( Hero hero ) {
			DriedRoseCopy rose = hero.belongings.getItem( DriedRoseCopy.class );
	...
	public static class GhostHero extends NPC {
		...
		private DriedRoseCopy rose = null;
		
		public GhostHero(){
			super();
		}

		public GhostHero(DriedRoseCopy rose){
			super();
			this.rose = rose;
			updateRose();
			HP = HT;
		...
		private void updateRose(){
			if (rose == null) {
				rose = Dungeon.hero.belongings.getItem(DriedRoseCopy.class);
			}

			if (rose == null) return;
			HT = 20 + Dungeon.hero.lvl * 4;
		}
		...
		protected Char chooseEnemy() {
			...
			if (enemy != null && Dungeon.level.mobs.contains(enemy)
					&& Dungeon.level.distance(enemy.pos, Dungeon.hero.pos) <= 10){
				blink( enemy.pos );
				return enemy;
			} else {
				return null;
		...
		public int damageRoll() {
			...
			if (rose != null && rose.weapon != null){
				...
				dmg += Random.NormalIntRange(Dungeon.hero.lvl * 2, Dungeon.hero.lvl * 5);
			}
	...
	private static class WndGhostHero extends Window{
		...
		WndGhostHero(final DriedRoseCopy rose){
			...
			btnWeapon = new WndBlacksmith.ItemButton(){
				...
				protected void onClick() {
					if (rose.weapon != null){
						...
						GameScene.selectItem(new WndBag.Listener() {
							...
							public void onSelect(Item item) {
								if (!(item instanceof MeleeWeapon || item instanceof Boomerang)) {
									...
									GLog.w( Messages.get(DriedRoseCopy.WndGhostHero.class, "cant_unique"));
									hide();
								} else if (!item.isIdentified()) {
									GLog.w( Messages.get(DriedRoseCopy.WndGhostHero.class, "cant_unidentified"));
									hide();
								} else if (item.cursed) {
									GLog.w( Messages.get(DriedRoseCopy.WndGhostHero.class, "cant_cursed"));
									hide();
								} else if (((MeleeWeapon)item).STRReq() > rose.ghostStrength()) {
									GLog.w( Messages.get(DriedRoseCopy.WndGhostHero.class, "cant_strength"));
									hide();
								} else {
						...
						}, WndBag.Mode.WEAPON, Messages.get(DriedRoseCopy.WndGhostHero.class, "weapon_prompt"));
					}
				}
			};
			...
			btnArmor = new WndBlacksmith.ItemButton(){
				...
				protected void onClick() {
					if (rose.armor != null){
						...
						}, WndBag.Mode.ARMOR, Messages.get(DriedRoseCopy.WndGhostHero.class, "armor_prompt"));
					}
				}
			};
