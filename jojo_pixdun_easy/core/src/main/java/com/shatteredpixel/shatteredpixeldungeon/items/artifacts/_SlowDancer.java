...
import com.shatteredpixel.shatteredpixeldungeon.sprites._HorseSprite;
...
public class _SlowDancer extends Artifact {
...
*/
		...
		if (ghost == null){
			if (charge < chargeCap) {
				...
				if (charge >= chargeCap) {
					charge = chargeCap;
					partialCharge = 0;
					GLog.p(Messages.get(_SlowDancer.class, "charged"));
				}
			}
		} else {
	...
	public class roseRecharge extends ArtifactBuff {
		...
		public boolean act() {
			...
			if (charge < chargeCap && !cursed && (lock == null || lock.regenOn())) {
				partialCharge += 1f; //500 turns to a full charge
				if (partialCharge > 1){
					charge += 5;
					partialCharge--;
					if (charge == chargeCap){
	...
	/*public static class Petal extends Item {
		...
		public boolean doPickUp( Hero hero ) {
			_SlowDancer rose = hero.belongings.getItem( _SlowDancer.class );
	...
	public static class GhostHero extends NPC {
		...
		{
			spriteClass = _HorseSprite.class;
		...
		private _SlowDancer rose = null;

		public GhostHero(){
			super();
		}

		public GhostHero(_SlowDancer rose){
			super();
			this.rose = rose;
			updateRose();
			HP = HT;
		...
		private void updateRose(){
			if (rose == null) {
				rose = Dungeon.hero.belongings.getItem(_SlowDancer.class);
			}

			//same dodge as the hero
			defenseSkill = (Dungeon.hero.lvl+4);
			if (rose == null) return;
			HT = 20 + 2 * Dungeon.hero.lvl;
		}
		...
		public int damageRoll() {
			...
			if (rose != null && rose.weapon != null){
				...
				dmg += Random.NormalIntRange(Dungeon.hero.lvl / 2, Dungeon.hero.lvl * 2);
			}
	...
	private static class WndGhostHero extends Window{
		...
		WndGhostHero(final _SlowDancer rose){
